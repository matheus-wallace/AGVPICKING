...
- [ ] 2.3 O código já para e fecha o TTS no `onStop`; falta a validação instrumentada/no
  Galaxy do motor Android, incluindo reprodução em curso.
...
- [x] 4.1 Executar `./gradlew testDebugUnitTest`, `./gradlew assembleDebug` e
  `./gradlew lintDebug`. **113 testes, 0 falhas; build e lint concluídos com sucesso.**
